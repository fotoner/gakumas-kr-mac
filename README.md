...
- `ios/GakumasLocalifyIOS_KR.dylib` 파일 — 디시 학원마스 갤러리 한글패치 공지에서 받아 `ios/` 폴더에 놓으세요 (**저장소에 포함돼있지 않음**, [공지 링크](https://gall.dcinside.com/mgallery/board/view/?id=gakumas&no=86599))
...
- **No license** — 이 저장소는 명시적 오픈소스 라이선스를 부여하지 않습니다 (기본 "All Rights Reserved" 상태). **개인 비상업적 사용만 권장**합니다. 상용 이용, 대규모 재배포, 유료 서비스화는 지양해주세요.
...
- 의존 라이브러리 (`Dobby` Apache-2.0, `Tyilo/insert_dylib`)는 각자 원 라이선스를 따릅니다. `make setup` 시 공식 저장소에서 clone됩니다.
